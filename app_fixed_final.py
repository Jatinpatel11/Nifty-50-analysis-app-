...
def create_price_charts_plotly(price_data):
    try:
        ...
        st.error(f"Error creating price charts with Plotly: {e}")
...
def create_price_charts_matplotlib(price_data):
    try:
        ...
        st.error(f"Error creating price charts with Matplotlib: {e}")
        st.error(traceback.format_exc())
        return None
...
def main():
    ...
    # Price Analysis page
        ...
        if price_data is None:
            st.error("Price data is not available. Please check data files or internet connection.")
            return
        ...
        with col1:
            start_date = st.date_input(
                "Start Date",
                value=price_data.index[-90].date(),
                min_value=price_data.index[0].date(),
                max_value=price_data.index[-1].date()
            )
        
        with col2:
            end_date = st.date_input(
                "End Date",
                value=price_data.index[-1].date(),
                min_value=price_data.index[0].date(),
                max_value=price_data.index[-1].date()
            )
        
        # Convert start_date and end_date to pandas Timestamp for proper indexing
        start_date = pd.Timestamp(start_date)
        end_date = pd.Timestamp(end_date)
        
        # Filter data based on date range
        filtered_data = price_data.loc[start_date:end_date]
        ...
        with col1:
            ...
            try:
                ...
                one_month_data = filtered_data.loc[filtered_data.index >= one_month_ago]
                if not one_month_data.empty:
                    one_month_price = one_month_data['Close'].iloc[0]
                ...
                st.warning(f"Could not calculate 1-month return: {e}")
        
        with col2:
            # 3-month return
            try:
                three_month_ago = filtered_data.index[-1] - pd.DateOffset(months=3)
                three_month_data = filtered_data.loc[filtered_data.index >= three_month_ago]
                if not three_month_data.empty:
                    three_month_price = three_month_data['Close'].iloc[0]
                    current_price = filtered_data['Close'].iloc[-1]
                ...
                st.warning(f"Could not calculate 3-month return: {e}")
        
        with col3:
            # 6-month return
            try:
                six_month_ago = filtered_data.index[-1] - pd.DateOffset(months=6)
                six_month_data = filtered_data.loc[filtered_data.index >= six_month_ago]
                if not six_month_data.empty:
                    six_month_price = six_month_data['Close'].iloc[0]
                    current_price = filtered_data['Close'].iloc[-1]
                ...
                st.warning(f"Could not calculate 6-month return: {e}")
        ...
        with col1:
            # Volatility chart
            if 'Volatility_20' in filtered_data.columns:
        ...
        try:
            ...
            st.error(f"Error calculating support and resistance levels: {e}")
            st.error(traceback.format_exc())
