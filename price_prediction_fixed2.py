...
class PricePredictionModel:
    ...
    def preprocess_data(self, prediction_horizon=5, sequence_length=60, test_size=0.2):
        ...
        for col in data.columns:
            if col not in ['Target']:  # Don't fill target variable yet
                data[col] = data[col].fillna(method='ffill')
                # If there are still NaN values at the beginning, use backward fill
                data[col] = data[col].fillna(method='bfill')
        
        # Now drop rows with NaN in target variable (can't predict these)
        ...
        if len(X) == 0:
            print("ERROR: No data available after preprocessing. Check for NaN values or data issues.")
            return None, None, None, None
    ...
    def run_all_models(self, prediction_horizon=5, sequence_length=60):
        ...
        # Preprocess data
        X_train, X_test, y_train, y_test = self.preprocess_data(prediction_horizon=prediction_horizon, sequence_length=sequence_length)
        
        # Check if preprocessing was successful
        if X_train is None:
            print("Preprocessing failed. Cannot build models.")
            return None
